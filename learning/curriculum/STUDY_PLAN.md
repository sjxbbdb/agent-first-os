...
如果你对二进制、CPU、内存和 I/O 完全陌生，可在阶段 0 与阶段 1 之间插入哈工大《计算机组成原理》BV1Xu411r7Vz 的少量预习：只看系统简介、基本组成、总线、主存、I/O 概览、指令系统和 CPU 结构，约 8–10 小时；跳过流水线、控制器细节和考研习题。
...
建议每周保持“看、做、讲”三段比例，而不是连续刷视频：

```text
2 小时：看指定章节并写三条自己的问题
4–8 小时：完成本阶段最小实验和一次独立变体
1 小时：运行故障注入，录下日志并口头解释
```

单阶段视频通常控制在 12 小时以内，阅读控制在 10 小时以内；如果仍然看不懂，先把问题带到课堂由老师补讲，不继续堆叠新课程。编码、调试和故障注入时间应至少不低于观看与阅读时间之和。
...
视频不应超过每阶段投入的三分之一，其余时间用于编码、调试、故障注入和解释。按每周 7–11 小时估算，阶段 0–3 约需 2–3 个月，阶段 4–8 约需 4–6 个月，阶段 9–13 约需 5–8 个月，阶段 14 是持续迭代。个人基础、每周时间和返工次数会显著改变总时长，是否进入下一阶段只看 Gate，不看日历。
