...
`include/agent_runtime_protocol.h` 将 Runtime 与 Supervisor 的控制事件别名为
现有 `IpcMessage` 定宽 envelope，并定义 `START`、`HEARTBEAT`、`CHECKPOINT`
和 `ACK` opcode。当前 bounded slice 使用 envelope 的 `sequence` 字段和
`words[0]` 传递一个序号；task identity、digest 等语义字段尚未进入该 ABI。
它同样只经由普通 IPC 传输，不授予 capability，也不直接调用设备。BIOS/QEMU
与 OVMF 的 `g9-*-runtime-service-test.sh` 会让两个真实 Ring 3 进程按
`START → HEARTBEAT → ACK → CHECKPOINT` 完成这条 bounded 生命周期；这不等同
于通用服务生成、持久化 checkpoint 或远程模型。
