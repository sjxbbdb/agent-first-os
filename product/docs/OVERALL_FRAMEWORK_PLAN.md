...
初始 M0–M7 审计发现过共享 2 MiB 页表、BIOS GDT 指针和停机式 `SYS_EXIT`。当前基线已修复其中的启动交接与 bootstrap 隔离问题：BootInfo v2、kernel-owned GDT/TSS、4 KiB 页表、NX/W^X 叶权限、page-table walk，以及固定 Ring 3 fixture 的 per-process CR3 已有证据；但还没有通用 ELF 进程地址空间、完整调度退出、跨进程 IPC/共享内存、Supervisor 原生启动、UEFI、virtio 或端到端策略/Agent 闭环。新的主机侧 G2/G3/G4/G7/G8/G9 原语和协议测试不能替代这些内核/硬件集成。
