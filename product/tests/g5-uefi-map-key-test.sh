...
out_dir="$repo_root/build/uefi-map-key"
esp_dir="$out_dir/esp"
log="$out_dir/qemu-map-key.log"
mkdir -p "$out_dir"

KERNEL_CFLAGS_EXTRA=-DAGENT_OS_TEST_RING3 \
    bash "$repo_root/product/tools/build-bios.sh" >/dev/null
UEFI_BUILD_DIR="$out_dir" \
UEFI_CFLAGS_EXTRA=-DAGENT_OS_TEST_FORCE_MAP_KEY_FAILURE \
...
echo 'PASS: injected stale UEFI map key is recovered by GetMemoryMap retry before kernel_entry'
