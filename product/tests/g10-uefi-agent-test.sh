...
out_dir="$repo_root/build/uefi-agent"
esp_dir="$out_dir/esp"
log="$out_dir/qemu-agent.log"

KERNEL_CFLAGS_EXTRA=-DAGENT_OS_TEST_RING3 \
USER_NASMFLAGS_EXTRA=-DAGENT_OS_TEST_AGENT_LOOP \
...
grep -Fq 'AGENT RING3 ACTION COMMITTED' "$log"
grep -Fq 'SCHEDULER idle - all tasks exited' "$log"
echo 'PASS: OVMF shared kernel_entry runs the Ring 3 Agent/Policy token loop to completion'
